<?php
    ...
    function resetCurrentProduct() { // redefine o valor do produto atual, quantidade e observação, caso o usuário saia da tela de produto
    ...
    function noEditFallback() { // caso o usuário tente ir para a página de editar produto sem clicar para editar
        if (!isset($_SESSION['editar-produto'])) {
            echo "<script>window.location = 'carrinho.php'</script>";
        }
    }
    
    function myCartButton() { // aparece o botão do carrinho quando o usuário adicionar algo ao carrinho
        if (isset($_SESSION['pedido'])) {
            echo '<script>document.querySelector("nav").insertAdjacentHTML("afterbegin", `
            <a href="carrinho.php" class="link-carrinho">
                <div class="meu-carrinho-wrapper">
                    <div class="meu-carrinho">
                        <span>Meu carrinho</span>
                        <svg width="23" height="23" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M20.9127 7.12138C20.8246 6.99399 20.7069 6.88987 20.5697 6.81793C20.4325 6.74599 20.2799 6.70839 20.125 6.70834H7.02742L5.9215 4.05375C5.77649 3.70418 5.53095 3.40553 5.216 3.19567C4.90105 2.98582 4.53088 2.87421 4.15242 2.875H1.91663V4.79167H4.15242L8.69875 15.7023C8.77157 15.8769 8.89442 16.026 9.05182 16.1309C9.20923 16.2357 9.39415 16.2917 9.58329 16.2917H17.25C17.6496 16.2917 18.007 16.0435 18.1479 15.6707L21.0229 8.004C21.0773 7.85893 21.0956 7.70285 21.0764 7.54913C21.0572 7.39541 21.0011 7.24864 20.9127 7.12138ZM16.5858 14.375H10.2225L7.82667 8.625H18.7421L16.5858 14.375Z" fill="black"/>
                            <path d="M10.0625 20.125C10.8564 20.125 11.5 19.4814 11.5 18.6875C11.5 17.8936 10.8564 17.25 10.0625 17.25C9.26859 17.25 8.625 17.8936 8.625 18.6875C8.625 19.4814 9.26859 20.125 10.0625 20.125Z" fill="black"/>
                            <path d="M16.7709 20.125C17.5648 20.125 18.2084 19.4814 18.2084 18.6875C18.2084 17.8936 17.5648 17.25 16.7709 17.25C15.977 17.25 15.3334 17.8936 15.3334 18.6875C15.3334 19.4814 15.977 20.125 16.7709 20.125Z" fill="black"/>
                        </svg>
                    </div>
                </div>
            </a>
        ...
        echo "<button class=\"produto-wrapper\" value=" . $line["id_cardapio"] . " name='produto'>
            <div class=\"produto-img\" style=\"background-image: url(" . loadProductImage($conn, $id_cardapio) . "); background-size: cover;\"></div>
                ...
                <p>" . $line["descricao_produto"] . "</p>
                    <div class=\"dinheiro\">
                    <span class=\"valor_dinheiro\">R$ " . $line["preco_produto"] . "</span>";
    ...
    function loadProductImage($conn, $id) {
        ...
        return $imagem; // retorna a imagem do banco
    }
?>
