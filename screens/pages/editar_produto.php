    ...
    <main>
        ...
        <?php
            ...
            if (isset($_POST['remover'])) {
                array_splice($_SESSION['pedido'], $pos_produto_carrinho, 1); // remove este item do carrinho
                echo '<script>window.location = "carrinho.php"</script>"';
            }

            if (isset($_POST['editar-produto'])) { // edita as informações do produto o usuário clicar no botão para editar
                $quant_editada = $_POST['quant'];

                $_SESSION['pedido'][$pos_produto_carrinho]['quant'] = $quant_editada;

                if (isset($_SESSION['editar-observacao'])) {
                    $_SESSION['pedido'][$pos_produto_carrinho]['obs'] = $_SESSION['editar-observacao'];
                }

                echo '<script>window.location = "carrinho.php"</script>"';
            }

            $produto = $_SESSION['pedido'][$pos_produto_carrinho];
            $quant = $produto['quant'];

            $id_cardapio = $produto['id'];
        ...
        <div style="background-image: url(<?php echo loadProductImage($conn, $id_cardapio); ?>);" class="img-produto"></div>
        <div class="horizontal-line"></div>
        <div class="info-produto">
            <?php fillProduct($resultado); ?>
            <a href="editar_obs.php"><u>Adicionar uma observação</u></a>
        </div>
        ...
        <form method="post">
          <div id="quantidade">
            ...
            <input id="numero-quant" name="quant" value="<?php echo $quant;?>" readonly></span>
          ...
          <a href="carrinho.php">
            <button id="adicionar-carrinho" name="editar-produto" value="<?php echo $pos_produto_carrinho;?>">
          ...
          <button name="remover" id="remover">Remover este produto</button>
        </form>
    </main>
